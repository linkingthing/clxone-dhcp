...
func (p *Pool4Handler) Create(ctx *restresource.Context) (restresource.Resource, *resterror.APIError) {
	...
	if err := pool.Validate(); err != nil {
		return nil, resterror.NewAPIError(resterror.InvalidFormat,
			fmt.Sprintf("create pool4 params invalid: %s", err.Error()))
	}
	...
	if err := restdb.WithTx(db.GetDB(), func(tx restdb.Transaction) error {
		...
		return nil, resterror.NewAPIError(resterror.ServerError,
			fmt.Sprintf("create pool4 %s with subnet4 %s failed: %s",
...
func sendCreatePool4CmdToDHCPAgent(subnetID uint64, nodes []string, pool *resource.Pool4) error {
	nodesForSucceed, err := sendDHCPCmdWithNodes(true, nodes, dhcpservice.CreatePool4,
		pool4ToCreatePool4Request(subnetID, pool))
	if err != nil {
		if _, err := dhcpservice.GetDHCPAgentService().SendDHCPCmdWithNodes(
			nodesForSucceed, dhcpservice.DeletePool4,
			pool4ToDeletePool4Request(subnetID, pool)); err != nil {
			log.Errorf("create subnet4 %d pool4 %s failed, and rollback it failed: %s",
...
func (p *Pool4Handler) List(ctx *restresource.Context) (interface{}, *resterror.APIError) {
	...
	if err := restdb.WithTx(db.GetDB(), func(tx restdb.Transaction) error {
		...
		return tx.FillEx(&reservations,
			"select * from gr_reservation4 where id in (select distinct r4.id from gr_reservation4 r4, gr_pool4 p4 where r4.subnet4 = $1 and r4.subnet4 = p4.subnet4 and r4.ip_address >= p4.begin_address and r4.ip_address <= p4.end_address)",
			subnet.GetID())
	}); err != nil {
		return nil, resterror.NewAPIError(resterror.ServerError,
			fmt.Sprintf("list pool4s with subnet4 %s from db failed: %s",
...
func (p *Pool4Handler) Get(ctx *restresource.Context) (restresource.Resource, *resterror.APIError) {
	...
	if err := restdb.WithTx(db.GetDB(), func(tx restdb.Transaction) error {
		...
		return nil, resterror.NewAPIError(resterror.ServerError,
			fmt.Sprintf("get pool4 %s with subnet4 %s from db failed: %s",
...
func (p *Pool4Handler) Delete(ctx *restresource.Context) *resterror.APIError {
	...
	if err := restdb.WithTx(db.GetDB(), func(tx restdb.Transaction) error {
		...
		return resterror.NewAPIError(resterror.ServerError,
			fmt.Sprintf("delete pool4 %s with subnet4 %s failed: %s",
				pool.String(), subnet.GetID(), err.Error()))
	}
...
func sendDeletePool4CmdToDHCPAgent(subnetID uint64, nodes []string, pool *resource.Pool4) error {
	_, err := sendDHCPCmdWithNodes(true, nodes, dhcpservice.DeletePool4,
...
func (p *Pool4Handler) Update(ctx *restresource.Context) (restresource.Resource, *resterror.APIError) {
	...
	if err := restdb.WithTx(db.GetDB(), func(tx restdb.Transaction) error {
		...
		return nil, resterror.NewAPIError(resterror.ServerError,
			fmt.Sprintf("update pool4 %s with subnet4 %s failed: %s",
