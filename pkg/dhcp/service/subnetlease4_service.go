...
func (l *SubnetLease4Service) BatchDeleteLease4s(subnet string, leaseIds []string) error {
	for _, leaseId := range leaseIds {
		if _, err := gohelperip.ParseIPv4(leaseId); err != nil {
			return fmt.Errorf("subnet4 %s lease4 id %s is invalid: %v",
				subnet, leaseId, err.Error())
		}
	}

	if err := restdb.WithTx(db.GetDB(), func(tx restdb.Transaction) error {
		subnet4, err := getSubnet4FromDB(tx, subnet)
		if err != nil {
			return err
		}
		...
		return fmt.Errorf("batch delete lease4 with subnet4 %s failed: %s", subnet, err.Error())
	}
