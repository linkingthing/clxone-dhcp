...
func getResolvedRatios(ctx *MetricContext) (*resource.Dns, *resterror.APIError) {
	...
	if err != nil {
		return nil, resterror.NewAPIError(resterror.ServerError,
			fmt.Sprintf("get node %s resolved ratios failed: %s", ctx.NodeIP, err.Error()))
	}
	...
	for _, r := range resp.Data.Results {
		...
		case agentmetric.RcodeSuccess, agentmetric.RcodeReferral, agentmetric.RcodeNxrrset,
			agentmetric.RcodeSERVFAIL, agentmetric.RcodeFORMERR, agentmetric.RcodeNXDOMAIN,
			agentmetric.RcodeDuplicate, agentmetric.RcodeDropped, agentmetric.RcodeFailure:
